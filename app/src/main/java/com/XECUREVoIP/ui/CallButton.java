...
public class CallButton extends ImageView implements OnClickListener, AddressAware {
	...
	public void onClick(View v) {
		try {
			if (!XecureManager.getInstance().acceptCallIfIncomingPending()) {
				if (mAddress.getText().length() > 0) {
					XecureManager.getInstance().newOutgoingCall(mAddress);
				} else {
					if (XecurePreferences.instance().isBisFeatureEnabled()) {
						LinphoneCallLog[] logs = XecureManager.getLc().getCallLogs();
