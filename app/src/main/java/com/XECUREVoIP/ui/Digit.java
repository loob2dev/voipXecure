	...
	private class DialKeyListener implements OnClickListener, OnTouchListener, OnLongClickListener {
		...
		public void onClick(View v) {
			...
			if (mAddress != null) {
				...
				if(XecurePreferences.instance().getDebugPopupAddress() != null
						&& mAddress.getText().toString().equals(XecurePreferences.instance().getDebugPopupAddress())){
					displayDebugPopup();
				}
			}
		}
		...
		public void displayDebugPopup(){
			...
			if(XecurePreferences.instance().isDebugEnabled()) {
				alertDialog.setItems(getContext().getResources().getStringArray(R.array.popup_send_log), new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
						if(which == 0){
							XecurePreferences.instance().setDebugEnabled(false);
				...
				alertDialog.setItems(getContext().getResources().getStringArray(R.array.popup_enable_log), new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
						if(which == 0) {
							XecurePreferences.instance().setDebugEnabled(true);
		...
		public boolean onTouch(View v, MotionEvent event) {
			...
			if (event.getAction() == MotionEvent.ACTION_DOWN && !mIsDtmfStarted) {
				XecureManager.getInstance().playDtmf(getContext().getContentResolver(), mKeyCode);
				mIsDtmfStarted = true;
			} else {
		...
		public boolean onLongClick(View v) {
			...
			if(id == R.id.Digit1 && lc.getCalls().length == 0){
				String voiceMail = XecurePreferences.instance().getVoiceMailUri();
				mAddress.getEditableText().clear();
				if(voiceMail != null){
