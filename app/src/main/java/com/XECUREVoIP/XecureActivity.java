...
import com.XECUREVoIP.account.AboutFragment;
import com.XECUREVoIP.account.AccountPreferencesFragment;
import com.XECUREVoIP.assistant.AssistantActivity;
import com.XECUREVoIP.assistant.RemoteProvisioningLoginActivity;
import com.XECUREVoIP.call.CallActivity;
import com.XECUREVoIP.call.CallIncomingActivity;
import com.XECUREVoIP.call.CallOutgoingActivity;
import com.XECUREVoIP.call.DialerFragment;
import com.XECUREVoIP.chat.ChatFragment;
import com.XECUREVoIP.chat.ChatListFragment;
import com.XECUREVoIP.chat.ChatUtils.XecureChatRoom;
import com.XECUREVoIP.compatibility.Compatibility;
import com.XECUREVoIP.contacts.ContactDetailsFragment;
import com.XECUREVoIP.contacts.ContactEditorFragment;
import com.XECUREVoIP.contacts.ContactsListFragment;
import com.XECUREVoIP.contacts.ContactsManager;
import com.XECUREVoIP.contacts.ExportContactsActivity;
import com.XECUREVoIP.contacts.ImportContactsActivity;
import com.XECUREVoIP.purchase.InAppPurchaseActivity;
...
import static com.XECUREVoIP.XecureActivity.ChatRoomContainer.createChatroomContainer;

public class XecureActivity extends XecureGenericActivity implements OnClickListener, ContactPicked, ActivityCompat.OnRequestPermissionsResultCallback {
	...
	private static XecureActivity instance;

	private StatusFragment statusFragment;
	private TextView missedCalls, missedChats;
	private RelativeLayout contacts, history, dialer, chat, passSetting;
	private View contacts_selected, history_selected, dialer_selected, chat_selected, passSetting_selected;
	...
	public static final XecureActivity instance() {
		if (instance != null)
			return instance;
		throw new RuntimeException("XecureActivity not instantiated yet");
	}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (XecurePreferences.instance().isProvisioningLoginViewEnabled()) {
			Intent wizard = new Intent();
			wizard.setClass(this, RemoteProvisioningLoginActivity.class);
			wizard.putExtra("Domain", XecureManager.getInstance().wizardLoginViewDomain);
			startActivity(wizard);
			finish();
			return;
		}else if (XecurePreferences.instance().getAccountCount() == 0){
			startActivity(new Intent().setClass(this, AssistantActivity.class));
			finish();
			return;
		}else if (savedInstanceState == null && (useFirstLoginActivity && XecureManager.getLcIfManagerNotDestroyedOrNull() != null
				&& XecurePreferences.instance().isFirstLaunch())) {
			if (XecurePreferences.instance().getAccountCount() > 0) {
		...
		mListener = new LinphoneCoreListenerBase(){
			...
			public void registrationState(LinphoneCore lc, LinphoneProxyConfig proxy, LinphoneCore.RegistrationState state, String smessage) {
				...
				if(getResources().getBoolean(R.bool.use_phone_number_validation)
					...
					if (state.equals(RegistrationState.RegistrationOk)) {
						XecureManager.getInstance().isAccountWithAlias();
					}
				}
			...
			public void callState(LinphoneCore lc, LinphoneCall call, LinphoneCall.State state, String message) {
				if (state == State.IncomingReceived) {
					startActivity(new Intent(XecureActivity.instance(), CallIncomingActivity.class));
				} else if (state == State.OutgoingInit || state == State.OutgoingProgress) {
					startActivity(new Intent(XecureActivity.instance(), CallOutgoingActivity.class));
				} else if (state == State.CallEnd || state == State.Error || state == State.CallReleased) {
					resetClassicMenuLayoutAndGoBackToCallIfStillRunning();
				}

				int missedCalls = XecureManager.getLc().getMissedCallsCount();
				displayMissedCalls(missedCalls);
			}
		};

		int missedCalls = (XecureManager.isInstanciated()) ? XecureManager.getLc().getMissedCallsCount() : 0;
		displayMissedCalls(missedCalls);
		...
		if (XecureManager.isInstanciated()) {
			XecureManager.getLc().setDeviceRotation(rotation);
		}
	...
	private void initButtons() {
		...
		passSetting = (RelativeLayout) findViewById(R.id.setting);
		...
		passSetting_selected = findViewById(R.id.setting_select);
...
//			checkAndRequestWriteContactsPermission();
		...
		case CHAT:
			...
			break;
		case CHANGE_PASS:
			fragment = new PassSettingFragment();
			break;
		default:
			break;
		}
	...
	public void displayHistoryDetail(String sipUri, LinphoneCallLog log) {
		...
		XecureContact c = ContactsManager.getInstance().findContactFromAddress(lAddress);

		String displayName = c != null ? c.getFullName() : XecureUtils.getAddressDisplayName(sipUri);
	...
	public void displayContact(XecureContact contact, boolean chatOnly) {
		Fragment fragment2 = getFragmentManager().findFragmentById(R.id.fragmentContainer2);
	...
	public void displayContactEditor(String sipAddress){
		Bundle extras = new Bundle();
		extras.putBoolean("EditOnClick", true);
		extras.putString("NewSipAdress", sipAddress);
	...
	public void displayAssistant() {
		startActivity(new Intent(XecureActivity.this, AssistantActivity.class));
	}

	public void displayLockSettings() {
		startActivity(new Intent(XecureActivity.this, ChangeSecurityActivity.class));
	}

	public void displayImportContacts() {
        startActivity(new Intent(XecureActivity.this, ImportContactsActivity.class));
    }

    public void displayExportContacts() {
		startActivity(new Intent(XecureActivity.this, ExportContactsActivity.class));
	}

	public void displayInapp() {
		startActivity(new Intent(XecureActivity.this, InAppPurchaseActivity.class));
	}

	public int getUnreadMessageCount() {
		int count = 0;
//		LinphoneChatRoom[] chats = XecureManager.getLc().getChatRooms();
//		for (LinphoneChatRoom chatroom : chats) {
//			count += chatroom.getUnreadMessagesCount();
//		}
		ArrayList<XecureChatRoom> chatRooms = XecureManager.getInstance().getXecureChatRooms();
		for (XecureChatRoom chatRoom: chatRooms){
			count += chatRoom.getUnreadMessagesCount();
		}
		return count;
	}
	...
	public void displayChat(String sipUri, String message, String fileUri) {
		...
		Bundle extras = new Bundle();
		extras.putString("entryId", sipUri);

		XecureService.instance().resetMessageNotifCount();
	...
	public void onClick(View v) {
		...
		if (id == R.id.history) {
			...
			XecureManager.getLc().resetMissedCallsCount();
			...
			changeCurrentFragment(FragmentsAvailable.CHANGE_PASS, null);
			passSetting_selected.setVisibility(View.VISIBLE);
		}
	}
	...
	private void resetSelection() {
		...
		passSetting_selected.setVisibility(View.GONE);
	}
...
//	}
	...
	public void removeFromChatList(String sipUri) {
		LinphoneChatRoom chatroom = XecureManager.getLc().getOrCreateChatRoom(sipUri);
	...
	public void displayMissedCalls(final int missedCallsCount) {
		if (missedCallsCount > 0) {
			...
			if (XecureManager.isInstanciated()) XecureManager.getLc().resetMissedCallsCount();
	...
	public Dialog displayImportDialog(Context context, String text){
	...
	public Dialog displayWrongPasswordDialog(final String username, final String realm, final String domain){
		...
		retry.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View view) {
				...
				XecureManager.getLc().addAuthInfo(authInfo);
...
//		changeCurrentFragment(FragmentsAvailable.DIALER, extras);
		...
		XecureManager.AddressType address = new AddressText(this, null);
		address.setDisplayedName(name);
		address.setText(number);
		XecureManager.getInstance().newOutgoingCall(address);
	}
	...
	public void resetClassicMenuLayoutAndGoBackToCallIfStillRunning() {
		...
		if (XecureManager.isInstanciated() && XecureManager.getLc().getCallsNb() > 0) {
			LinphoneCall call = XecureManager.getLc().getCalls()[0];
			if (call.getState() == LinphoneCall.State.IncomingReceived) {
				startActivity(new Intent(XecureActivity.this, CallIncomingActivity.class));
			} else {
	...
	public void editContact(XecureContact contact)
	{
		Bundle extras = new Bundle();
		extras.putSerializable("Contact", contact);
		changeCurrentFragment(FragmentsAvailable.CONTACT_EDITOR, extras);
	...
	public void editContact(XecureContact contact, String sipAddress)
	{
		Bundle extras = new Bundle();
		extras.putSerializable("Contact", contact);
	...
	public void quit() {
		finish();
//		stopService(new Intent(Intent.ACTION_MAIN).setClass(this, XecureService.class));
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (resultCode == Activity.RESULT_FIRST_USER && requestCode == SETTINGS_ACTIVITY) {
			...
			if (Compatibility.canDrawOverlays(this)) {
				XecurePreferences.instance().enableOverlay(true);
			}
		} else {
...
//		checkAndRequestPermission(Manifest.permission.WRITE_CONTACTS, 0);
	...
	public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
		...
		switch (requestCode) {
			...
			case PERMISSIONS_READ_EXTERNAL_STORAGE_DEVICE_RINGTONE:
				...
				XecurePreferences.instance().enableDeviceRingtone(enableRingtone);
				XecureManager.getInstance().enableDeviceRingtone(enableRingtone);
				break;
	...
	protected void onStart() {
		...
		if (ringtone != PackageManager.PERMISSION_GRANTED) {
			if (XecurePreferences.instance().firstTimeAskingForPermission(Manifest.permission.READ_EXTERNAL_STORAGE) || ActivityCompat.shouldShowRequestPermissionRationale(this, Manifest.permission.READ_EXTERNAL_STORAGE)) {
				Log.i("[Permission] Asking for read external storage for ring tone");
				permissionsList.add(Manifest.permission.READ_EXTERNAL_STORAGE);
			}
		}
		if (readPhone != PackageManager.PERMISSION_GRANTED) {
			if (XecurePreferences.instance().firstTimeAskingForPermission(Manifest.permission.READ_PHONE_STATE) || ActivityCompat.shouldShowRequestPermissionRationale(this, Manifest.permission.READ_PHONE_STATE)) {
				Log.i("[Permission] Asking for read phone state");
				permissionsList.add(Manifest.permission.READ_PHONE_STATE);
			}
		}
		if (contacts != PackageManager.PERMISSION_GRANTED) {
			if (XecurePreferences.instance().firstTimeAskingForPermission(Manifest.permission.READ_CONTACTS) || ActivityCompat.shouldShowRequestPermissionRationale(this, Manifest.permission.READ_CONTACTS)) {
	...
	protected void onResume() {
		super.onResume();
		if (!XecureService.isReady()) {
			startService(new Intent(Intent.ACTION_MAIN).setClass(this, XecureService.class));
		}

		LinphoneCore lc = XecureManager.getLcIfManagerNotDestroyedOrNull();
		if (lc != null) {
			lc.addListener(mListener);
			if (!XecureService.instance().displayServiceNotification()) {
				lc.refreshRegisters();
			}
		}
		...
		displayMissedCalls(XecureManager.getLc().getMissedCallsCount());

		XecureManager.getInstance().changeStatusToOnline();

		if (getIntent().getIntExtra("PreviousActivity", 0) != CALL_ACTIVITY && !doNotGoToCallActivity) {
			if (XecureManager.getLc().getCalls().length > 0) {
				LinphoneCall call = XecureManager.getLc().getCalls()[0];
	...
	protected void onNewIntent(Intent intent) {
		...
		if (extras != null && extras.getBoolean("GoToChat", false)) {
			...
			if (XecureManager.getLc().getCalls().length > 0) {
				...
				if (XecureUtils.getCallsInState(XecureManager.getLc(), incoming).size() > 0) {
					if (CallActivity.isInstanciated()) {
						CallActivity.instance().startIncomingCallActivity();
					} else {
						startActivity(new Intent(this, CallIncomingActivity.class));
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if (keyCode == KeyEvent.KEYCODE_BACK) {
			if (currentFragment == FragmentsAvailable.DIALER
				...
				boolean isBackgroundModeActive = XecurePreferences.instance().isBackgroundModeEnabled();
				if (!isBackgroundModeActive) {
					stopService(new Intent(Intent.ACTION_MAIN).setClass(this, XecureService.class));
					finish();
				} else if (XecureUtils.onKeyBackGoHome(this, keyCode, event)) {
					return true;
				}
			}
		}
...
//		sideMenuItems.add(getResources().getString(R.string.menu_assistant));
		...
		sideMenuItems.add(getResources().getString(R.string.menu_security));
		sideMenuItems.add(getResources().getString(R.string.menu_import_contact));
		sideMenuItems.add(getResources().getString(R.string.menu_export_contact));
//		if(getResources().getBoolean(R.bool.enable_in_app_purchase)){
...
//		sideMenuItems.add(getResources().getString(R.string.menu_about));
		...
		sideMenuContent.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
			}
		});
		...
		sideMenuItemList.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
				...
				if (sideMenuItemList.getAdapter().getItem(i).toString().equals(getString(R.string.menu_about))) {
					XecureActivity.instance().displayAbout();
				}
				if (sideMenuItemList.getAdapter().getItem(i).toString().equals(getString(R.string.menu_assistant))) {
					XecureActivity.instance().displayAssistant();
				}
				if (sideMenuItemList.getAdapter().getItem(i).toString().equals(getString(R.string.menu_security))) {
					XecureActivity.instance().displayLockSettings();
				}
				if (sideMenuItemList.getAdapter().getItem(i).toString().equals(getString(R.string.menu_import_contact))) {
                    XecureActivity.instance().displayImportContacts();
				}
				if (sideMenuItemList.getAdapter().getItem(i).toString().equals(getString(R.string.menu_export_contact))) {
					XecureActivity.instance().displayExportContacts();
				}
				if(getResources().getBoolean(R.bool.enable_in_app_purchase)){
					if (sideMenuItemList.getAdapter().getItem(i).toString().equals(getString(R.string.inapp))) {
						XecureActivity.instance().displayInapp();
					}
				}
	...
	private void displayMainAccount(){
		...
		LinphoneProxyConfig proxy = XecureManager.getLc().getDefaultProxyConfig();
		if(proxy == null) {
			...
			String strAddress = proxy.getAddress().asStringUriOnly().replace("sip:", "account:");
			address.setText(strAddress);
			displayName.setText(XecureUtils.getAddressDisplayName(proxy.getAddress()));
			...
			defaultAccount.setOnClickListener(new OnClickListener() {
				@Override
				public void onClick(View view) {
					XecureActivity.instance().displayAccountSettings(XecurePreferences.instance().getDefaultAccountIndex());
					openOrCloseSideMenu(false);
				}
			});
		}
	...
	public void refreshAccounts(){
		if (XecureManager.getLc().getProxyConfigList() != null &&
				XecureManager.getLc().getProxyConfigList().length > 1) {
			...
			accountsList.setOnItemClickListener(new AdapterView.OnItemClickListener() {
				...
				public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
					if(view != null && view.getTag() != null) {
						...
						XecureActivity.instance().displayAccountSettings(position);
					}
					openOrCloseSideMenu(false);
				}
			});
	...
	class AccountsListAdapter extends BaseAdapter {
		...
		public void refresh(){
			proxy_list = new ArrayList<LinphoneProxyConfig>();
			for(LinphoneProxyConfig proxyConfig : XecureManager.getLc().getProxyConfigList()){
				if(proxyConfig != XecureManager.getLc().getDefaultProxyConfig()){
		...
		public View getView(final int position, View convertView, ViewGroup parent) {
			...
			int nbAccounts = XecurePreferences.instance().getAccountCount();
			int accountIndex = 0;

			for (int i = 0; i < nbAccounts; i++) {
				String username = XecurePreferences.instance().getAccountUsername(i);
				String domain = XecurePreferences.instance().getAccountDomain(i);
	...
	private void isTrialAccount() {
		if(XecureManager.getLc().getDefaultProxyConfig() != null && XecurePreferences.instance().getInappPopupTime() != null) {
			...
			}, XecurePreferences.instance().getAccountUsername(XecurePreferences.instance().getDefaultAccountIndex()), XecurePreferences.instance().getAccountHa1(XecurePreferences.instance().getDefaultAccountIndex()));
		}
	}
	...
	public void displayInappNotification(String date) {
		...
		if (XecurePreferences.instance().getInappPopupTime() != null && Long.parseLong(XecurePreferences.instance().getInappPopupTime()) > now.getTime()) {
			return;
		} else {
			long newDate = now.getTime() + getResources().getInteger(R.integer.time_between_inapp_notification);
			XecurePreferences.instance().setInappPopupTime(String.valueOf(newDate));
		}
		if(isTrialAccount){
			XecureService.instance().displayInappNotification(String.format(getString(R.string.inapp_notification_trial_expire), date));
		} else {
			XecureService.instance().displayInappNotification(String.format(getString(R.string.inapp_notification_account_expire), date));
		}
	...
	public Handler mNotifyReceiceMesage = new Handler(){
		@Override
		public void handleMessage(Message msg) {
			XecureActivity.this.updateMissedChatCount();
		}
	};
