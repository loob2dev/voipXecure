...
import com.XECUREVoIP.contacts.ContactsUpdatedListener;
...
public class HistoryListFragment extends Fragment implements OnClickListener, OnItemClickListener, ContactsUpdatedListener {
	...
	public void refresh() {
		mLogs = Arrays.asList(XecureManager.getLc().getCallLogs());
	}
	...
	public void displayFirstLog(){
		if (mLogs != null && mLogs.size() > 0) {
			...
			if (log.getDirection() == CallDirection.Incoming) {
				XecureActivity.instance().displayHistoryDetail(mLogs.get(0).getFrom().toString(), mLogs.get(0));
			} else {
				XecureActivity.instance().displayHistoryDetail(mLogs.get(0).getTo().toString(), mLogs.get(0));
			}
		} else {
	...
	private void removeCallLogs(){
		...
		for(int i=0; i<size; i++) {
			if(historyList.isItemChecked(i)){
				...
				XecureManager.getLc().removeCallLog(log);
			}
		}
	}
	...
	public void onResume() {
		...
		if (XecureActivity.isInstanciated()) {
			XecureActivity.instance().selectMenu(FragmentsAvailable.HISTORY_LIST);
			XecureActivity.instance().hideTabBar(false);
			XecureActivity.instance().displayMissedCalls(0);
		}
	...
	public void onContactsUpdated() {
		if (!XecureActivity.isInstanciated() || XecureActivity.instance().getCurrentFragment() != FragmentsAvailable.HISTORY_LIST)
			return;
	...
	class CallHistoryAdapter extends BaseAdapter {
		...
		public View getView(final int position, View convertView, ViewGroup parent) {
			...
			if (c != null) {
				...
				XecureUtils.setThumbnailPictureFromUri(XecureActivity.instance(), holder.contactPicture, c.getThumbnailUri());
			} else {
				holder.contactPicture.setImageBitmap(ContactsManager.getInstance().getDefaultAvatarBitmap());
			}

			if (displayName == null) {
				holder.contact.setText(XecureUtils.getAddressDisplayName(sipUri));
			} else {
			...
			if (isEditMode) {
				...
				holder.detail.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						if (XecureActivity.isInstanciated()) {
							XecureActivity.instance().displayHistoryDetail(sipUri, log);
						}
					}
				});
