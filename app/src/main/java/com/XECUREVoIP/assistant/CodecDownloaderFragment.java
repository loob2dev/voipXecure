...
public class CodecDownloaderFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		final OpenH264DownloadHelper codecDownloader = XecureManager.getInstance().getOpenH264DownloadHelper();
					...
					public void run() {
						if (current <= max) {
							...
							if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP_MR1) {
								enabledH264(true);
								XecureManager.getLc().reloadMsPlugins(AssistantActivity.instance().getApplicationInfo().nativeLibraryDir);
	...
	private void enabledH264(boolean enable) {
		...
		for (PayloadType pt : XecureManager.getLc().getVideoCodecs()) {
			if (pt.getMime().equals("H264")) h264 = pt;
		}

		if (h264 != null) {
			try {
				XecureManager.getLc().enablePayloadType(h264, enable);
