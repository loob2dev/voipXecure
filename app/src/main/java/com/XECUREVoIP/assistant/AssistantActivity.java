...
private static AssistantActivity instance;
					...
					if (address != null && address.asString().equals(cfg.getAddress().asString()) ) {
						if (state == RegistrationState.RegistrationOk) {
							...
							if (getResources().getBoolean(R.bool.use_phone_number_validation)
									...
									&& XecureManager.getLc().getDefaultProxyConfig() != null) {
								loadAccountCreator(cfg).isAccountUsed();
							} else {
								Intent intent = new Intent(AssistantActivity.this, GenSecurityActivity.class);
	...
	private LinphoneAccountCreator loadAccountCreator(LinphoneProxyConfig cfg) {
		...
		LinphoneProxyConfig cfgTab[] = XecureManager.getLc().getProxyConfigList();
		accountCreator.setListener(this);
		int n = -1;
	...
	public void onClick(View v) {
		...
		if (id == R.id.assistant_cancel) {
			...
			if (getResources().getBoolean(R.bool.assistant_cancel_move_to_back)) {
				...
				XecurePreferences.instance().firstLaunchSuccessful();
				startActivity(new Intent().setClass(this, XecureActivity.class));
				finish();
			}
	...
	public void configureLinphoneProxyConfig(LinphoneAccountCreator accountCreator) {
		...
		try {
            ...
            if (identity == null || accountCreator.getUsername() == null) {
                XecureUtils.displayErrorAlert(getString(R.string.error), this);
                return;
            }
			...
			if (XecurePreferences.instance() != null)
				XecurePreferences.instance().setPushNotificationEnabled(true);
			...
			XecureManager.getInstance().subscribeFriendList(getResources().getBoolean(R.bool.use_friendlist_subscription));
	...
	public void displayLoginLinphone(String username, String password) {
		fragment = new XecureLoginFragment();
	...
	private void launchDownloadCodec() {
		if (XecureManager.getLc().downloadOpenH264Enabled()) {
	...
	public void saveCreatedAccount(String username, String userid, String password, String displayname, String ha1, String prefix, String domain, TransportType transport) {
		...
		username = XecureUtils.getDisplayableUsernameFromAddress(username);
		domain = XecureUtils.getDisplayableUsernameFromAddress(domain);
		...
		XecurePreferences.AccountBuilder builder = new XecurePreferences.AccountBuilder(XecureManager.getLc())
 	...
 	public void displayRegistrationInProgressDialog() {
		if(XecureManager.getLc().isNetworkReachable()) {
	...
	public void displayAssistantLinphoneLogin(String phone, String dialcode) {
		XecureLoginFragment fragment = new XecureLoginFragment();
		newAccount = true;
		Bundle extras = new Bundle();
		extras.putString("Phone", phone);
		extras.putString("Dialcode", dialcode);
...
////						XecureManager.getLc().removeProxyConfig(XecureManager.getLc().getDefaultProxyConfig());
////						XecurePreferences.instance().resetDefaultProxyConfig();
////						XecureManager.getLc().refreshRegisters();
//						XecurePreferences.instance().deleteAccount(XecurePreferences.instance().getAccountCount() - 1);
//						dialog.dismiss();
//					}
//				});
		builder.setMessage("The information about the account you have entered is incorrect. Try again!")
				.setTitle("Wrong Information")
				.setNegativeButton(getString(R.string.ok), new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int id) {
//						XecureManager.getLc().removeProxyConfig(XecureManager.getLc().getDefaultProxyConfig());
//						XecurePreferences.instance().resetDefaultProxyConfig();
//						XecureManager.getLc().refreshRegisters();
	...
	public void success() {
		boolean needsEchoCalibration = XecureManager.getLc().needsEchoCalibration();
	...
	private void goToLinphoneActivity() {
		...
		startActivity(new Intent().setClass(this, XecureActivity.class).putExtra("isNewProxyConfig", true));
		finish();
	}
	...
	public void restartApplication() {
		...
		Intent mStartActivity = new Intent(this, XecureLauncherActivity.class);
