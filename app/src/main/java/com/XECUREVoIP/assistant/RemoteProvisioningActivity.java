...
public class RemoteProvisioningActivity extends Activity {
	...
	protected void onResume() {
		...
		XecurePreferences.instance().setContext(this);
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				if (configUriParam == null) {
					if (!XecurePreferences.instance().isFirstRemoteProvisioning()) {
					...
					if (getResources().getBoolean(R.bool.display_confirmation_popup_after_first_configuration)
							&& !XecurePreferences.instance().isFirstRemoteProvisioning()) {
						mHandler.post(new Runnable() {
							@Override
							public void run() {
	...
	private void setRemoteProvisioningAddressAndRestart(final String configUri) {
		...
		XecurePreferences.instance().setContext(this); // Needed, else the next call will crash
		XecurePreferences.instance().setRemoteProvisioningUrl(configUri);
	...
	private void goToLinphoneActivity() {
		if (XecureService.isReady()) {
			XecureService.instance().setActivityToLaunchOnIncomingReceived("XecureLauncherActivity");
			//finish(); // To prevent the user to come back to this page using back button
			startActivity(new Intent().setClass(this, XecureLauncherActivity.class));
		} else {
			finish();
		}
	}
