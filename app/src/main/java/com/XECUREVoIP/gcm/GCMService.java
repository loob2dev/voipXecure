...
public class GCMService extends GCMBaseIntentService {
	...
	private void initLogger(Context context) {
		XecurePreferences.instance().setContext(context);
	...
	protected void onMessage(Context context, Intent intent) {
		...
		} else if (XecureManager.isInstanciated() && XecureManager.getLc().getCallsNb() == 0) {
			UIThreadDispatcher.dispatch(new Runnable(){
				@Override
				public void run() {
					if (XecureManager.isInstanciated() && XecureManager.getLc().getCallsNb() == 0){
						XecureManager.getLc().setNetworkReachable(false);
						XecureManager.getLc().setNetworkReachable(true);
					}
				}
			});
		}
	...
	protected void onUnregistered(Context context, String regId) {
		...
		UIThreadDispatcher.dispatch(new Runnable(){
			@Override
			public void run() {
				XecurePreferences.instance().setPushNotificationRegistrationID(null);
			}
		});
	}
