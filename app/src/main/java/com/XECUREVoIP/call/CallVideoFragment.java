...
import com.XECUREVoIP.compatibility.CompatibilityScaleGestureDetector;
import com.XECUREVoIP.compatibility.CompatibilityScaleGestureListener;
...
public class CallVideoFragment extends Fragment implements OnGestureListener, OnDoubleTapListener, CompatibilityScaleGestureListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View view;
		if (XecureManager.getLc().hasCrappyOpenGL()) {
		...
		androidVideoWindowImpl = new AndroidVideoWindowImpl(mVideoView, mCaptureView, new AndroidVideoWindowImpl.VideoWindowListener() {
			public void onVideoRenderingSurfaceReady(AndroidVideoWindowImpl vw, SurfaceView surface) {
				mVideoView = surface;
				XecureManager.getLc().setVideoWindow(vw);
			}
			...
			public void onVideoPreviewSurfaceReady(AndroidVideoWindowImpl vw, SurfaceView surface) {
				...
				XecureManager.getLc().setPreviewWindow(mCaptureView);
			}
	...
	public void switchCamera() {
		try {
			int videoDeviceId = XecureManager.getLc().getVideoDevice();
			videoDeviceId = (videoDeviceId + 1) % AndroidCameraConfiguration.retrieveCameras().length;
			XecureManager.getLc().setVideoDevice(videoDeviceId);
	...
	public void onResume() {
		...
		if (XecurePreferences.instance().isOverlayEnabled()) {
			XecureService.instance().destroyOverlay();
		}
		if (androidVideoWindowImpl != null) {
			synchronized (androidVideoWindowImpl) {
				XecureManager.getLc().setVideoWindow(androidVideoWindowImpl);
			}
		}
	...
	public void onPause() {
		if (androidVideoWindowImpl != null) {
			synchronized (androidVideoWindowImpl) {
				 ...
				 */
				XecureManager.getLc().setVideoWindow(null);
			}
		}
		if (XecurePreferences.instance().isOverlayEnabled()) {
			XecureService.instance().createOverlay();
		}
    ...
    public boolean onScale(CompatibilityScaleGestureDetector detector) {
    	...
    	LinphoneCall currentCall = XecureManager.getLc().getCurrentCall();
    	if (currentCall != null) {
	...
	public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
		if (XecureUtils.isCallEstablished(XecureManager.getLc().getCurrentCall())) {
			if (mZoomFactor > 1) {
				...
				XecureManager.getLc().getCurrentCall().zoomVideo(mZoomFactor, mZoomCenterX, mZoomCenterY);
				return true;
			}
		}
