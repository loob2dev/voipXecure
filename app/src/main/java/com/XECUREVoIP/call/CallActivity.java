...
import com.XECUREVoIP.BluetoothManager;
...
import com.XECUREVoIP.ui.Numpad;
...
public class CallActivity extends XecureGenericActivity implements OnClickListener, ActivityCompat.OnRequestPermissionsResultCallback {
		...
		mListener = new LinphoneCoreListenerBase() {
			...
			public void callState(LinphoneCore lc, final LinphoneCall call, LinphoneCall.State state, String message) {
				if (XecureManager.getLc().getCallsNb() == 0) {
					if (status != null) {
						XecureService.instance().removeSasNotification();
				...
				if (state == State.IncomingReceived) {
					...
					if(XecureManager.getLc().getCurrentCall() != null) {
					...
					if(XecurePreferences.instance().isVideoEnabled()){
					...
					boolean videoEnabled = XecurePreferences.instance().isVideoEnabled();
					if (!videoEnabled) {
...
//        			else if (remoteVideo && !XecureManager.getLc().isInConference() && autoAcceptCameraPolicy) {
...
//        			}
				...
				refreshIncallUi();
				transfer.setEnabled(XecureManager.getLc().getCurrentCall() != null);
			}
		...
		if (findViewById(R.id.fragmentContainer) != null) {
			...
			if (XecureManager.getLc().getCallsNb() > 0) {
				...
				if (XecureUtils.isCallEstablished(call)) {
					enableAndRefreshInCallActions();
				}
			}
			if (savedInstanceState != null) {
				...
				isSpeakerEnabled = XecureManager.getLc().isSpeakerEnabled();
				isMicMuted = XecureManager.getLc().isMicMuted();
			}

			Fragment callFragment;
			if (isVideoEnabled(XecureManager.getLc().getCurrentCall())) {
				callFragment = new CallVideoFragment();
				videoCallFragment = (CallVideoFragment) callFragment;
				displayVideoCall(false);
				XecureManager.getInstance().routeAudioToSpeaker();
	...
	protected void onSaveInstanceState(Bundle outState) {
		outState.putBoolean("Speaker", XecureManager.getLc().isSpeakerEnabled());
		outState.putBoolean("Mic", XecureManager.getLc().isMicMuted());
	...
	private void initUI() {
		...
		createInCallStats();
		XecureManager.getInstance().changeStatusToOnThePhone();
	}
	...
	public void checkAndRequestPermission(String permission, int result) {
		...
		if (permissionGranted != PackageManager.PERMISSION_GRANTED) {
			if (XecurePreferences.instance().firstTimeAskingForPermission(permission) || ActivityCompat.shouldShowRequestPermissionRationale(this, permission)) {
	...
	public void createInCallStats() {
		...
		initCallStatsRefresher(XecureManager.getLc().getCurrentCall(), findViewById(R.id.incall_stats));
	}
	...
	public void refreshInCallActions() {
		if (!XecurePreferences.instance().isVideoEnabled() || isConferenceRunning) {
			enabledVideoButton(false);
		} else {
			if(video.isEnabled()) {
				if (isVideoEnabled(XecureManager.getLc().getCurrentCall())) {
	...
	private void enableAndRefreshInCallActions() {
		...
		if(XecureManager.getLc().isInConference()) {
			confsize = XecureManager.getLc().getConferenceSize() - (XecureManager.getLc().isInConference() ? 1 : 0);
		}

		//Enabled transfer button
		if(isTransferAllowed  && !XecureManager.getLc().soundResourcesLocked())
			enabledTransferButton(true);

		//Enable conference button
		if(XecureManager.getLc().getCallsNb() > 1 && XecureManager.getLc().getCallsNb() > confsize && !XecureManager.getLc().soundResourcesLocked()) {
			enabledConferenceButton(true);
		} else {
			enabledConferenceButton(false);
		}

		addCall.setEnabled(XecureManager.getLc().getCallsNb() < XecureManager.getLc().getMaxCalls() && !XecureManager.getLc().soundResourcesLocked());
		options.setEnabled(!getResources().getBoolean(R.bool.disable_options_in_call) && (addCall.isEnabled() || transfer.isEnabled()));

		if(XecureManager.getLc().getCurrentCall() != null && XecurePreferences.instance().isVideoEnabled() && !XecureManager.getLc().getCurrentCall().mediaInProgress()) {
			enabledVideoButton(true);
		} else {
			enabledVideoButton(false);
		}
		if(XecureManager.getLc().getCurrentCall() != null && !XecureManager.getLc().getCurrentCall().mediaInProgress()){
	...
	public void onClick(View v) {
		...
		if (id == R.id.video) {
			...
			if (camera == PackageManager.PERMISSION_GRANTED) {
				disableVideo(isVideoEnabled(XecureManager.getLc().getCurrentCall()));
			} else {
			...
			pauseOrResumeCall(XecureManager.getLc().getCurrentCall());
		}
	...
	private void disableVideo(final boolean videoDisabled) {
		final LinphoneCall call = XecureManager.getLc().getCurrentCall();
		if (call == null) {
			return;
		}

		if (videoDisabled) {
			LinphoneCallParams params = XecureManager.getLc().createCallParams(call);
			params.setVideoEnabled(false);
			XecureManager.getLc().updateCall(call, params);
		} else {
			videoProgress.setVisibility(View.VISIBLE);
			if (call.getRemoteParams() != null && !call.getRemoteParams().isLowBandwidthEnabled()) {
				XecureManager.getInstance().addVideo();
			} else {
	...
	private void showAudioView() {
		if (XecureManager.getLc().getCurrentCall() != null) {
			if (!isSpeakerEnabled) {
				XecureManager.getInstance().enableProximitySensing(true);
			}
		}
	...
	private void showVideoView() {
		...
		XecureManager.getInstance().enableProximitySensing(false);
...
//		Hiding controls to let displayVideoCallControlsIfHidden add them plus the callback
	...
	protected void toggleSpeaker() {
		...
		if (XecureManager.getLc().getCurrentCall() != null) {
			XecureManager.getInstance().enableProximitySensing(!isSpeakerEnabled);
		}
		if (isSpeakerEnabled) {
			XecureManager.getInstance().routeAudioToSpeaker();
			speaker.setImageResource(R.drawable.speaker_selected);
	...
	public void pauseOrResumeCall(LinphoneCall call) {
		LinphoneCore lc = XecureManager.getLc();
		if (call != null && XecureManager.getLc().getCurrentCall() == call) {
			lc.pauseCall(call);
			if (isVideoEnabled(XecureManager.getLc().getCurrentCall())) {
	...
	public void resetControlsHidingCallBack() {
		...
		if (isVideoEnabled(XecureManager.getLc().getCurrentCall()) && mControlsHandler != null) {
	...
	public void acceptCallUpdate(boolean accept) {
		...
		if (accept) {
			...
			XecureManager.getLc().enableVideo(true, true);
		}

		try {
			XecureManager.getLc().acceptCallUpdate(call, params);
	...
	public void showStatusBar() {
		...
		//findViewById(R.id.fragmentContainer).setPadding(0, XecureUtils.pixelsToDpi(getResources(), 40), 0, 0);
	}
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if (XecureUtils.onKeyVolumeAdjust(keyCode)) return true;
		if (XecureUtils.onKeyBackGoHome(this, keyCode, event)) return true;
	...
	private void setContactInformation(TextView contactName, ImageView contactPicture, LinphoneAddress lAddress) {
		XecureContact lContact  = ContactsManager.getInstance().findContactFromAddress(lAddress);
		if (lContact == null) {
			contactName.setText(XecureUtils.getAddressDisplayName(lAddress));
			contactPicture.setImageBitmap(ContactsManager.getInstance().getDefaultAvatarBitmap());
		} else {
			contactName.setText(lContact.getFullName());
			XecureUtils.setImagePictureFromUri(contactPicture.getContext(), contactPicture, lContact.getPhotoUri(), lContact.getThumbnailUri());
		}
	}
	...
	public void refreshCallList(Resources resources) {
		isConferenceRunning = XecureManager.getLc().isInConference();
		List<LinphoneCall> pausedCalls = XecureUtils.getCallsInState(XecureManager.getLc(), Arrays.asList(State.PausedByRemote));

		//MultiCalls
		if(XecureManager.getLc().getCallsNb() > 1){
			callsList.setVisibility(View.VISIBLE);
		}
		...
		if(XecureManager.getLc().getCurrentCall() != null) {
			...
			if(isVideoEnabled(XecureManager.getLc().getCurrentCall()) && !isConferenceRunning && pausedCalls.size() == 0) {
				displayVideoCall(false);
			} else {
				displayAudioCall();
			}
		...
		if(callsList != null) {
			...
			for (LinphoneCall call : XecureManager.getLc().getCalls()) {
				if (call.isInConference() && !isConferenceRunning) {
					isConfPaused = true;
					index++;
				} else {
					if (call != XecureManager.getLc().getCurrentCall() && !call.isInConference()) {
	...
	private void enterConference() {
		XecureManager.getLc().addAllToConference();
	}
	...
	private void displayConferenceParticipant(int index, final LinphoneCall call){
		...
		XecureContact lContact  = ContactsManager.getInstance().findContactFromAddress(call.getRemoteAddress());
		if (lContact == null) {
	...
	private String getEncoderText(String mime){
		...
		if (ret == null){
			org.linphone.mediastream.Factory msfactory = XecureManager.getLc().getMSFactory();
		...
		mTask = new TimerTask() {
			...
			public void run() {
				...
				mHandler.post(new Runnable() {
					@Override
					public void run() {
						if (XecureManager.getLcIfManagerNotDestroyedOrNull() == null) return;
						synchronized(XecureManager.getLc()) {
