...
import com.XECUREVoIP.purchase.Purchasable;
...
public class XecurePreferences {
	private static final int LINPHONE_CORE_RANDOM_PORT = -1;
	private static XecurePreferences instance;
	private Context mContext;
	private String basePath;

	public static final synchronized XecurePreferences instance() {
		if (instance == null) {
			instance = new XecurePreferences();
		}
		return instance;
	}

	private XecurePreferences() {
	...
	private String getString(int key) {
		if (mContext == null && XecureManager.isInstanciated()) {
			mContext = XecureManager.getInstance().getContext();
		}
	...
	private LinphoneCore getLc() {
		...
		return XecureManager.getLcIfManagerNotDestroyedOrNull();
	}
	...
	public LpConfig getConfig() {
		...
		if (!XecureManager.isInstanciated()) {
			...
			return LinphoneCoreFactory.instance().createLpConfig(XecureManager.getInstance().mLinphoneConfigFile);
		}
		return null;
	}
	...
	private void setAccountPassword(int n, String password, String ha1) {
		...
		if(user != null && domain != null) {
			if (XecureManager.getLc().getAuthInfosList().length >= n && XecureManager.getLc().getAuthInfosList()[n] != null) {
				userid = getAccountUserId(n);
				realm = getAccountRealm(n);
				XecureManager.getLc().removeAuthInfo(XecureManager.getLc().getAuthInfosList()[n]);
			}
	...
	public void setAccountEnabled(int n, boolean enabled) {
		LinphoneProxyConfig prxCfg = getProxyConfig(n);
		if (prxCfg == null) {
			XecureUtils.displayErrorAlert(getString(R.string.error), mContext);
			return;
		}
		prxCfg.edit();
...
// End of call settings
	...
	public void setTunnelHost(String host) {
		...
		if(config != null) {
			config.setHost(host);
			XecureManager.getInstance().initTunnelFromConf();
		}
	}
	...
	public String getActivityToLaunchOnIncomingReceived() {
		return getConfig().getString("app", "incoming_call_activity", "XecureActivity");
	}
