...
import com.XECUREVoIP.ui.LedPreference;
...
public class SettingsFragment extends PreferencesListFragment {
	...
	public void onCreate(Bundle bundle) {
		...
		mListener = new LinphoneCoreListenerBase() {
			...
			public void ecCalibrationStatus(LinphoneCore lc, final EcCalibratorStatus status, final int delayMs, Object data) {
				...
				if (status == EcCalibratorStatus.DoneNoEcho) {
					...
					XecurePreferences.instance().setEchoCancellation(false);
					...
					XecurePreferences.instance().setEchoCancellation(true);
					((AudioManager)getActivity().getSystemService(Context.AUDIO_SERVICE)).setMode(AudioManager.MODE_NORMAL);
					Log.i("Set audio mode on 'Normal'");
		...
		PreferenceCategory call_category = (PreferenceCategory)findPreference("call_category");
		Preference pref_sipinfo_dtmf_key = findPreference(getString(R.string.pref_sipinfo_dtmf_key));
		Preference pref_rfc2833_dtmf_key = findPreference(getString(R.string.pref_rfc2833_dtmf_key));
		Preference pref_auto_answer_time_key = findPreference(getString(R.string.pref_auto_answer_time_key));
		call_category.removePreference(pref_sipinfo_dtmf_key);
		call_category.removePreference(pref_rfc2833_dtmf_key);
		call_category.removePreference(pref_auto_answer_time_key);

		PreferenceCategory vieo_category = (PreferenceCategory)findPreference("video_key");
		Preference pref_preferred_video_size_key = findPreference(getString(R.string.pref_preferred_video_size_key));
		Preference pref_preferred_video_fps_key = findPreference(getString(R.string.pref_preferred_video_fps_key));
		Preference pref_bandwidth_limit_key = findPreference(getString(R.string.pref_bandwidth_limit_key));
		Preference pref_overlay_key = findPreference(getString(R.string.pref_overlay_key));
		vieo_category.removePreference(pref_preferred_video_size_key);
		vieo_category.removePreference(pref_preferred_video_fps_key);
		vieo_category.removePreference(pref_bandwidth_limit_key);
		vieo_category.removePreference(pref_overlay_key);

		PreferenceScreen pref_video_title = (PreferenceScreen) findPreference(getString(R.string.pref_video_key));
		PreferenceCategory codecs = (PreferenceCategory) findPreference(getString(R.string.pref_video_codecs_key));

		pref_video_title.removePreference(codecs);
	...
	private void removePreviousPreferencesFile() {
		...
		XecureUtils.recursiveFileRemoval(dir);
	}
	...
	private void initSettings() {
		...
		findPreference(getString(R.string.pref_add_account_key)).setOnPreferenceClickListener(new OnPreferenceClickListener() {
			...
			public boolean onPreferenceClick(Preference preference) {
				...
				XecureActivity.instance().displayAccountSettings(nbAccounts);
				return true;
			}
		});
		hidePreference(findPreference(getString(R.string.pref_advanced_key)));
		hidePreference(findPreference(getString(R.string.pref_chat_key)));
		hidePreference(findPreference(getString(R.string.pref_voice_mail_key)));
		hidePreference(findPreference(getString(R.string.pref_dialer_call_key)));
		hidePreference(findPreference(getString(R.string.pref_media_encryption_key)));
		hidePreference(findPreference(getString(R.string.pref_audio_key)));
		hidePreference(findPreference(getString(R.string.pref_network_key)));
	}
	...
	private void hideSettings() {
		...
		if (!Version.isVideoCapable() || !XecureManager.getLcIfManagerNotDestroyedOrNull().isVideoSupported()) {
		...
		if (!XecureManager.getLc().isTunnelAvailable()) {
		...
		if (!XecureManager.getLc().upnpAvailable()) {
	...
	private void initTunnelSettings() {
		if (!XecureManager.isInstanciated() || !XecureManager.getLc().isTunnelAvailable()) {
			return;
		}
		...
		if (!getResources().getBoolean(R.bool.hide_accounts)) {
			...
			for (int i = 0; i < nbAccounts; i++) {
				...
				account.setOnPreferenceClickListener(new OnPreferenceClickListener() {
					public boolean onPreferenceClick(Preference preference) {
						XecureActivity.instance().displayAccountSettings(accountId);
						return false;
					}
				});
	...
	private void updateAccountLed(final LedPreference me, final String username, final String domain, boolean enabled) {
		...
		if (XecureManager.getLcIfManagerNotDestroyedOrNull() != null) {
			for (LinphoneProxyConfig lpc : XecureManager.getLc().getProxyConfigList()) {
	...
	private void initializePreferredVideoSizePreferences(ListPreference pref) {
		List<CharSequence> entries = new ArrayList<CharSequence>();
		List<CharSequence> values = new ArrayList<CharSequence>();
		for (String name : XecureManager.getLc().getSupportedVideoSizes()) {
			entries.add(name);
			values.add(name);
		}
...
//			codec.setChecked(lc.isPayloadTypeEnabled(pt));
			...
			codec.setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
				...
				public boolean onPreferenceChange(Preference preference, Object newValue) {
					...
					try {
						XecureManager.getLcIfManagerNotDestroyedOrNull().enablePayloadType(pt, enable);
					} catch (LinphoneCoreException e) {
						Log.e(e);
					}
					return true;
			...
			public boolean onPreferenceClick(Preference preference) {
				synchronized (SettingsFragment.this) {
					...
					if (recordAudio == PackageManager.PERMISSION_GRANTED) {
						...
						XecureActivity.instance().checkAndRequestRecordAudioPermissionForEchoCanceller();
					}
				}
				return true;
			}
		...
		findPreference(getString(R.string.pref_echo_tester_key)).setOnPreferenceClickListener(new OnPreferenceClickListener() {
			...
			public boolean onPreferenceClick(Preference preference) {
				synchronized (SettingsFragment.this) {
					int recordAudio = getActivity().getPackageManager().checkPermission(Manifest.permission.RECORD_AUDIO, getActivity().getPackageName());
					if (recordAudio == PackageManager.PERMISSION_GRANTED) {
						if (XecureManager.getInstance().getEchoTesterStatus())
							stopEchoTester();
						else
							startEchoTester();
					} else {
						XecureActivity.instance().checkAndRequestRecordAudioPermissionsForEchoTester();
					}
				}
				return true;
			}
	...
	public void startEchoTester() {
		...
		try {
			if (XecureManager.getInstance().startEchoTester() > 0) {
	...
	public void stopEchoTester() {
		Preference preference = findPreference(getString(R.string.pref_echo_tester_key));
		try {
			if (XecureManager.getInstance().stopEchoTester() > 0) {
	...
	public void startEchoCancellerCalibration() {
		try {
			if (XecureManager.getInstance().getEchoTesterStatus())
				stopEchoTester();
			XecureManager.getInstance().startEcCalibration(mListener);
		} catch (LinphoneCoreException e) {
			Log.e(e);
		}
	}
	...
	private void initVideoSettings() {
		...
		final LinphoneCore lc = XecureManager.getLcIfManagerNotDestroyedOrNull();

		final OpenH264DownloadHelper mCodecDownloader = XecureManager.getInstance().getOpenH264DownloadHelper();
					...
					try {
						if (lc.downloadOpenH264Enabled()) {
							if (enable && Version.getCpuAbis().contains("armeabi-v7a") && !Version.getCpuAbis().contains("x86")
								...
								mCodecDownloader.setOpenH264HelperListener(XecureManager.getInstance().getOpenH264HelperListener());
								mCodecDownloader.setUserData(0, XecureManager.getInstance().getContext());
								mCodecDownloader.setUserData(1, codec);

								AlertDialog.Builder builder = new AlertDialog.Builder(XecureManager.getInstance().getContext());
...
//		((CheckBoxPreference) findPreference(getString(R.string.pref_video_automatically_accept_video_key))).setChecked(mPrefs.shouldAutomaticallyAcceptVideoRequests());
		((CheckBoxPreference) findPreference(getString(R.string.pref_video_automatically_accept_video_key))).setChecked(true);
	...
	private void setVideoPreferencesListener() {
		...
		findPreference(getString(R.string.pref_overlay_key)).setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
			...
			public boolean onPreferenceChange(Preference preference, Object newValue) {
				boolean enable = (Boolean) newValue;
				if (enable) {
					if (XecureActivity.instance().checkAndRequestOverlayPermission()) {
	...
	public void enableDeviceRingtone(boolean enabled) {
		XecurePreferences.instance().enableDeviceRingtone(enabled);
		XecureManager.getInstance().enableDeviceRingtone(enabled);
	...
	private void setCallPreferencesListener() {
			...
			public boolean onPreferenceChange(Preference preference, Object newValue) {
				...
				if (use) {
					...
					if (readExternalStorage == PackageManager.PERMISSION_GRANTED) {
						...
						XecureManager.getInstance().enableDeviceRingtone(true);
					} else {
						XecureActivity.instance().checkAndRequestReadExternalStoragePermissionForDeviceRingtone();
					}
				} else {
					mPrefs.enableDeviceRingtone(false);
					XecureManager.getInstance().enableDeviceRingtone(false);
				}
		...
		findPreference(getString(R.string.pref_auto_answer_key)).setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
			...
			public boolean onPreferenceChange(Preference preference, Object newValue) {
				...
				if (use)
					mPrefs.setAutoAnswerTime(Integer.valueOf(0));
				/*if (use) {
	...
	private void setEncryptionZrtp() {
		XecureUtils.displayErrorAlert(getString(R.string.lime_encryption_enable_zrtp), XecureActivity.instance());
	...
	private void setAdvancedPreferencesListener() {
		...
		findPreference(getString(R.string.pref_friendlist_subscribe_key)).setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
			...
			public boolean onPreferenceChange(Preference preference, Object newValue) {
				...
				XecureManager.getInstance().subscribeFriendList(value);
				return true;
			}
		});
		...
		findPreference(getString(R.string.pref_service_notification_key)).setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
			...
			public boolean onPreferenceChange(Preference preference, Object newValue) {
				...
				if (value) {
					XecureService.instance().showServiceNotification();
				} else {
					XecureService.instance().hideServiceNotification();
				}
				return true;
			}
		});
	...
	@Override
	public void onPause() {
		if (XecureActivity.isInstanciated() && XecureManager.isInstanciated()) {
			if (XecureManager.getInstance().getEchoTesterStatus())
				stopEchoTester();
