...
public class BluetoothManager extends BroadcastReceiver {
	...
	public BluetoothManager() {
		...
		if (!ensureInit()) {
			Log.w("[Bluetooth] Manager tried to init but XecureService not ready yet...");
		}
		instance = this;
	}
	...
	public void initBluetooth() {
		if (!ensureInit()) {
			Log.w("[Bluetooth] Manager tried to init bluetooth but XecureService not ready yet...");
			return;
		}
	...
	private boolean ensureInit() {
		...
		if (mContext == null) {
			if (XecureService.isReady()) {
				mContext = XecureService.instance().getApplicationContext();
			} else {
				return false;
			}
		}
...
//				XecureManager.getInstance().audioStateChanged(AudioState.BLUETOOTH);
    			isScoConnected = true;
			} else if (state == AudioManager.SCO_AUDIO_STATE_DISCONNECTED) {
				Log.d("[Bluetooth] SCO state: disconnected");
//				XecureManager.getInstance().audioStateChanged(AudioState.SPEAKER);
