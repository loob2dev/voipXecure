...
public class InAppPurchaseActivity extends Activity implements InAppPurchaseListener, OnClickListener {
	...
	public void buyInapp(String username, Purchasable item){
		XecurePreferences.instance().setInAppPurchasedItem(item);
	...
	public void onPurchasedItemConfirmationQueryFinished(boolean success) {
		if (success) {
			...
			Purchasable item = XecurePreferences.instance().getInAppPurchasedItem();
			...
			}, XecurePreferences.instance().getAccountUsername(0), XecurePreferences.instance().getAccountHa1(0), getString(R.string.default_domain), item.getPayload(), item.getPayloadSignature());
		}
	}
